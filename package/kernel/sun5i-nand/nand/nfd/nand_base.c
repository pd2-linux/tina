...
int __init nand_init(void)
{
...
#if 1
    /* 获取card_line值 */
    type = script_parser_fetch("nand_para", "nand_used", &nand0_used_flag,sizeof(int));
    if(type)
    {
    ...
    nand_cache_level = 0;
    type = script_parser_fetch("nand_para", "nand_cache_level", &nand_cache_level,sizeof(int));
    if(type)
    {
...
#if 1
    /* 获取card_line值 */
    type = script_parser_fetch("nand0_para", "nand0_used", &nand0_used_flag,sizeof(int));
    if(type)
