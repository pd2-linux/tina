...
cdx_int32 ebml_read_element_id (MatroskaDemuxContext *matroska, cdx_uint32 *id, cdx_int32 *level_up)
{
    ...
    *id = matroska->peek_id  = (cdx_uint32)(total | (1 << (read * 7)));
    ...
    //the amount of levels in the hierarchy that the current element lies higher than the previous one.
    if (level_up)
	{
		CdxStreamT *pb = matroska->fp;
		cdx_uint64 pos = CdxStreamTell(pb);
...
cdx_int32 ebml_read_ascii (MatroskaDemuxContext *matroska, cdx_uint32 *id, cdx_int8 **str)
{
    ...
    // ebml strings are usually not 0-terminated, so we allocate one byte more, read the string and NULL-terminate it ourselves.
    if (size < 0) 
    {
 ...
 * 0 is success, < 0 is failure.
 *********************************************************/
cdx_int32 ebml_read_binary (MatroskaDemuxContext *matroska, cdx_uint32 *id, cdx_uint8 **binary, cdx_int32 *size, cdx_int32 offset)
{
    CdxStreamT *pb = matroska->fp;
    cdx_uint64 rlength;
    cdx_int32 res;
    ...
    while (res == 0)
    {
        ...
        switch (id) 
        {
            ...
            case CDX_EBML_ID_DOCTYPE: 
            {
				...
				if (strcmp((const char *)doctype, "matroska") && strcmp((const char *)doctype, "webm")) 
				{
					free(doctype);
...
cdx_int32 matroska_parse_info (MatroskaDemuxContext *matroska)
{
    ...
    matroska->segment_duration = (cdx_int64)(matroska->segment_duration_f * matroska->time_scale / AV_TIME_BASE);
...
cdx_int32 matroska_add_stream (MatroskaDemuxContext *matroska)
{
        ...
        switch (id) 
        {
                ...
                while (res == 0) 
                {
                    ...
                    switch (id) 
                    {
                        ...
                        case CDX_MATROSKA_ID_AUDIOSAMPLINGFREQ: 
                        {
                            ...
                            if (res < 0)
                                break;
                            audiotrack->internal_samplerate = audiotrack->samplerate = (cdx_int32)num;
                            break;
                        }
            ...
            case CDX_MATROSKA_ID_CODECID: 
            {
				...
				for(j=0; ff_mkv_codec_tags[j].id != CODEC_ID_NONE; j++){
					if(!strncmp((const char *)ff_mkv_codec_tags[j].str, (const char *)text, strlen((char*)ff_mkv_codec_tags[j].str))){
    ...
    if(track_name)
    {
    	...
    	else if(name_len<=ADECODER_MAX_LANG_CHAR_SIZE)
    	{
    		...
    		{
    			track->language[ADECODER_MAX_LANG_CHAR_SIZE-2-i] = track->language[ADECODER_MAX_LANG_CHAR_SIZE-name_len-1-i];
    		}
    ...
    while (res == 0)
    {
        ...
        switch (id) 
        {
            ...
            case CDX_MATROSKA_ID_POINTENTRY:
                ...
                // in the end, we hope to fill one entry with a timestamp, a file position and a tracknum
				...
				{
					if (idx.pos != (cdx_uint64) -1 && idx.time != (cdx_uint64) -1 && idx.track != (cdx_uint16) -1) {
            ...
            case CDX_MATROSKA_ID_SEEKENTRY: 
            {
                ...
                switch (seek_id)
                {
                    ...
                    case CDX_MATROSKA_ID_TAGS:
                    {
                        ...
                        // seek
                        if ((res = ebml_read_seek(matroska, seek_pos + matroska->segment_start)) < 0)
                            goto finish;

                        // we don't want to lose our seekhead level, so we add a dummy. This is a crude hack.
                        if (matroska->num_levels == EBML_MAX_DEPTH) {
...
cdx_int32 matroska_read_header (struct CdxMkvParser *p)
{
    ...
    // We now have a Matroska segment. Seeks are from the beginning of the segment, after the segment ID/length.
    if ((res = ebml_read_master(matroska, &id)) < 0)
        return res;
    ...
    while (res == 0) 
    {
        ...
        switch (id) 
        {
            ...
            case CDX_MATROSKA_ID_CLUSTER: {
                // Do not read the master - this will be done in the next call to matroska_read_packet.
                res = 1;
                break;
            }
...
#define max_as_search 23
        ...
        for (i = 0; i < matroska->num_tracks; i++) 
        {
            ...
            // Set the FourCC from the CodecID. This is the MS compatibility mode which stores a BITMAPINFOHEADER in the CodecPrivate.
            if ((codec_id == CODEC_ID_V_UNKNOWN) && (track->codec_priv_size >= 40) &&  (track->codec_priv != NULL))
            {
            ...
            // This is the MS compatibility mode which stores a WAVEFORMATEX in the CodecPrivate.
            else if ((codec_id == CODEC_ID_A_UNKNOWN) && (track->codec_priv_size >= 18) && (track->codec_priv != NULL)) {
                cdx_uint16 tag;
                //int ret;
				...
				*************************************/
				...
				audiotrack->bitrate     = (CDX_AV_RL32(track->codec_priv+8))*8; // it is bytes per seconds, so we should x 8
            ...
            else if (codec_id == CODEC_ID_TTA) 
            {
				...
				extradata[6] = audiotrack->channels & 0xff; extradata[7] = (audiotrack->channels>>8) & 0xff;
				extradata[8] = audiotrack->bitdepth & 0xff; extradata[9] = (audiotrack->channels>>8) & 0xff;
				extradata[10] = audiotrack->samplerate & 0xff; extradata[11] = (audiotrack->samplerate>>8) & 0xff;
				extradata[12] = (audiotrack->samplerate>>16) & 0xff; extradata[13] = (audiotrack->samplerate>>24) & 0xff;
                     ...
                     codec_id == CODEC_ID_SIPRO) 
            {
				...
				audiotrack->coded_framesize = (track->codec_priv[24]<<24) | (track->codec_priv[25]<<16) | (track->codec_priv[26]<<8) | (track->codec_priv[27]);
                ...
                if(track->codec_priv_size)
                	memcpy(extradata+32, track->codec_priv+extradata_offset, track->codec_priv_size);
            }
            ...
            if (track->type == MATROSKA_TRACK_TYPE_VIDEO) 
            {
				...
				st->sample_aspect_ratio = (st->height * videotrack->display_width)*1000 / (st->width * videotrack->display_height);
        ...
        for (i = 0; i < matroska->num_tracks; i++)
        {
			...
			if (track->type == MATROSKA_TRACK_TYPE_SUBTITLE &&
                (track->codec_id == CODEC_ID_TEXT || track->codec_id == CODEC_ID_SSA || track->codec_id == CODEC_ID_DVD_SUBTITLE || track->codec_id == CODEC_ID_PGS))
            {
				...
				else if(track->codec_id == CODEC_ID_SSA)
				{
					...
					if(ssa_header != 0)
					{
    					...
    					{
        					matroska->SubTitledata_encode_type[p->nhasSubTitle] = SUBTITLE_TEXT_FORMAT_UTF16BE;
    					}
    					else if(ssa_header[0] == 0xff && ssa_header[1] == 0xfe)
    					{
        					matroska->SubTitledata_encode_type[p->nhasSubTitle] = SUBTITLE_TEXT_FORMAT_UTF16LE;
    					}
    					else if(ssa_header[0] == 0xef && ssa_header[1] == 0xbb && ssa_header[2] == 0xbf)
    					{
        					matroska->SubTitledata_encode_type[p->nhasSubTitle] = SUBTITLE_TEXT_FORMAT_UTF8;
    					}
    				}
                ...
                else if(track->codec_id == CODEC_ID_DVD_SUBTITLE)
                {
                    ...
                    matroska->SubTitledata_encode_type[p->nhasSubTitle] = SUBTITLE_TEXT_FORMAT_UNKNOWN;
                }
				...
				{
					strcpy((char *)matroska->language[p->nhasSubTitle], (const char *)track->language);
				}
                else
                {
                    strncpy((char *)matroska->language[p->nhasSubTitle], (const char *)track->language, MAX_SUBTITLE_LANG_SIZE - 1);
		...
		if(p->hasSubTitle == 0)
		{
			for (i = 0; i < matroska->num_tracks; i++) {
				...
				if (track->type == MATROSKA_TRACK_TYPE_SUBTITLE && track->codec_id == CODEC_ID_DVD_SUBTITLE) 
				{
					matroska->SubTitleStream[p->nhasSubTitle] = track->stream_index;
					matroska->SubTitleTrack[p->nhasSubTitle] = i;
 ...
 * 0 is success, < 0 is failure.
 *********************************************************/
cdx_int32 ebml_read_binary_part (MatroskaDemuxContext *matroska, cdx_uint32 *id, cdx_uint8 **binary, cdx_int32 *size, cdx_int32 offset)
{
    CdxStreamT *pb = matroska->fp;
    cdx_uint64 rlength;
    cdx_int32 res;
	...
	*size = (cdx_int32)rlength;
		   ...
		   matroska->tracks[track_num-1]->type == MATROSKA_TRACK_TYPE_SUBTITLE )
	{
		...
		if (CdxStreamRead(pb, (cdx_uint8*)(*binary+matroska->sector_align+4), (cdx_uint32)(*size)-4) != (cdx_int32)(*size)-4)
		{
			//cdx_uint64 pos = CdxStreamTell(pb);
			return AVERROR(EIO);
		}
...
cdx_int32 matroska_parse_block_header(MatroskaDemuxContext *matroska)
{
...

	{
		...
		{
			if((matroska->cluster_time != INT64_MAX) && ((cdx_int64)(matroska->cluster_time + block_time)>=0))
			{
		...
		else if(matroska->chk_type == MATROSKA_TRACK_TYPE_VIDEO)
		{
	        ...
	        else
	        {
				if(matroska->is_keyframe == PKT_FLAG_KEY)
				{
					if((matroska->cluster_time != INT64_MAX) && (((cdx_int64)matroska->cluster_time + block_time)>=0))
					{
						cdx_int64 delta_pts = (matroska->cluster_time + block_time)*st->time_scale - st->pts - matroska->blk_duration;
						if(matroska->frame_num_between_kframe > 5)
						{
							if(delta_pts>0)
							{
								matroska->blk_duration += ((cdx_int32)delta_pts)/matroska->frame_num_between_kframe;
							}
							else if(delta_pts<0)
							{
								matroska->blk_duration -= ((cdx_int32)-delta_pts)/matroska->frame_num_between_kframe;
				...
				else
				{
			        ...
			        else
			        {
						...
						else
						{
							...
							{
								if((st->last_pts - st->pts) > 16000000&&(st->last_pts - st->pts) < 60000000)
								{
            ...
            switch ((flags & 0x06) >> 1) 
            {
                ...
                {   // EBML lacing
                    ...
                    for (n = 1; res == 0 && n < laces - 1; n++) {
                        cdx_int64 snum;
                        cdx_int32 r;
                        r = matroska_ebmlnum_sint (matroska->data_parse, matroska->size_parse, &snum);
                        if (r < 0) 
                        {
                            break;
                        }
...
cdx_int32 matroska_parse_blockgroup (MatroskaDemuxContext *matroska)
{
    ...
    while (res == 0) 
    {
        ...
        switch (id) 
        {
            ...
            case CDX_MATROSKA_ID_BLOCKREFERENCE: // We've found a reference, so not even the first frame in the lace is a key frame.
    ...
    while (res == 0) 
    {
        ...
        switch (id) 
        {
            ...
            case CDX_MATROSKA_ID_CLUSTERTIMECODE: 
            {
				...
				if((cdx_int64)matroska->cluster_time * matroska->time_scale > (matroska->segment_duration * AV_TIME_BASE))
				{
...
static void ra_bitcopy(cdx_uint8 *toPtr, cdx_uint32  ulToBufSize, cdx_uint8  *fromPtr, cdx_int32  bitOffsetTo,
                cdx_int32 bitOffsetFrom, cdx_int32 numBits)
{
    ...
    cdx_uint8 rightWord, leftWord, *offsetFrom, *offsetTo,
                  alignWord, endWord;
    cdx_uint8 leftMask[9] = {0, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff}; // Flawfinder: ignore
    cdx_uint8 rightMask[9] = {0, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f, 0xff}; // Flawfinder: ignore
    ...
    if (numBits>>3 == 0) // quick and easy if we have fewer than 8 bits to align
    {
        ...
        if (numMod >= offsetToModLeft)  // have more extra input bits than free space in current output byte
        {
       	...
       	else // have fewer input bits than free space in current output byte be careful not to overwrite extra bits already in output byte
        {
    ...
    if (bitOffsetFrom%4 == 0 && bitOffsetTo%4 == 0)	// byte-packing done here is optimized for the common case of nibble-alignment
    {
...
static void RASL_DeInterleave(cdx_uint8 *buf, cdx_uint32 ulBufSize, cdx_int32 type)
{
    ...
    if(codeBits%8 == 0)
    {
        ...
        for (fIn = 0; fIn < RASL_NFRAMES * RASL_NBLOCKS; fIn++)
        {
            ...
            //Note that when (fOut == fIn), the frame doesn't move, and if (fOut < fIn), we have swapped it already.
...
cdx_int32 matroska_parse_block_info(MatroskaDemuxContext *matroska)
{
             ...
             st->codec_id == CODEC_ID_ATRAC3)
	{
        ...
        if (st->codec_id == CODEC_ID_RA_288)
            ...
            for (x=0; x<w/sps; x++)
                memcpy(matroska->buf+sps*(h*x+((h+1)/2)*(y&1)+(y>>1)), matroska->data_parse+x*sps, sps);
    ...
    else if(st->codec_id == CODEC_ID_VORBIS)
    {
    	...
    	matroska->ogg_pageno++;
       	for(matroska->lace_cnt=0; matroska->lace_cnt < matroska->laces; matroska->lace_cnt++)
       	{
       		...
       		for(lacing_fill=0; lacing_fill*0xff<(cdx_int32)matroska->lace_size[matroska->lace_cnt]; lacing_fill++);
    		...
    		for(; pading_cnt<pading_len; pading_cnt++)
    		{
    			...
    			{
    				matroska->pkt_data[pading_cnt] = matroska->lace_size[matroska->lace_cnt] - (lacing_fill-1)*0xff;
    			}
    		}
    	}
...
/************************************************************************/
cdx_int32 matroska_find_keyframe(struct CdxMkvParser *p, cdx_int32 direction, cdx_int32 next_index, cdx_int32 ms)
{
	MatroskaDemuxContext *matroska = p->priv_data;
	...
	matroska->data_rdy = 0;
	matroska->parsing_cluster=0;
	st->pts = matroska->streams[p->AudioStreamIndex]->pts = st->index_entries[index].timestamp*AV_TIME_BASE;
    return 0;
}
...
static int find_next_pts(unsigned char* data, int data_size, int *tmp)
{
	...
	*tmp		 = 0;
	...
	for(pkt = data; pkt <= dataEnd-9; pkt ++)
	{
		...
		{
			//LOGD("...0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x", *(pkt+1), *(pkt+2), *(pkt+3), *(pkt+4), *(pkt+5), *(pkt+6), *(pkt+7), *(pkt+8), *(pkt+9));
...
static int mkv_seek_keyframe(struct CdxMkvParser *p, int timeInterval)
{
	...
	int     ret = 0;
	//int     pos_find = 0;
	CDX_LOGV("%d ms , %lld ms , FileEndPst %lld", timeInterval, matroska->segment_duration, matroska->fileEndPst);
	jump_file_pos = (matroska->fileEndPst/matroska->segment_duration)*timeInterval 
					+ ((matroska->fileEndPst%matroska->segment_duration)*timeInterval)/matroska->segment_duration;
...
reseek:
	...
	if(CdxStreamRead(matroska->fp, matroska->tmpbuf, matroska->tmpbufsize) < (int)matroska->tmpbufsize)
	{
...
static int MkvGetClusterTimeCode(struct CdxMkvParser *p, cdx_uint64* num)
{
	...
	while (res == 0) 
	{
		...
		CDX_LOGD("--- id = %x, total = %llx, read = %d, pos = %llx", id, total, read, CdxStreamTell(matroska->fp));
		if (res < 0) 
		{
			CDX_LOGE("--- id = %x", id);
			return -1;
		...
		switch (id) 
		{
			...
			case CDX_MATROSKA_ID_CLUSTERTIMECODE: 
			{
			    ...
			    *num = 0;
				...
				CDX_LOGD("--- timecode length = %lld, offset = %llx", rlength, CdxStreamTell(matroska->fp));
				return 0;
			}
...
static int MkvEstimateBR(struct CdxMkvParser *p)
{
	...
	{
		CdxStreamSeek(matroska->fp, offsetTmp+clusterLength-CdxStreamTell(matroska->fp)-7, SEEK_CUR);
	...
	{
		matroska->bitrate = (offset2-offset)/(clusterTime2-clusterTime)* AV_TIME_BASE / matroska->time_scale;
	}
    ...
    if(p->hasVideo)
    {
        ...
        switch (track->codec_id)
        {
            case CODEC_ID_MPEG4:
            {
    			...
    			p->vFormat.pCodecSpecificData = (void *)matroska->streams[p->VideoStreamIndex]->extradata;
    			p->vFormat.nCodecSpecificDataLen = matroska->streams[p->VideoStreamIndex]->extradata_size;
                break;
            }
			...
			if (track->type == MATROSKA_TRACK_TYPE_AUDIO) 
			{
			    ...
			    switch(track->codec_id)
			    {
			        ...
			        case CODEC_ID_WMAV2:
			            ...
			            aFormat->nCodecSpecificDataLen 	= matroska->streams[stream_index]->extradata_size;
			            aFormat->pCodecSpecificData 	= matroska->streams[stream_index]->extradata;
                        p->nhasAudio ++;
			            break;
...
#if 0
	// for aliyun 720p.mkv, whose video extradata is NULL, so we must get the first video frame as extradata
    if((p->vFormat.eCodecFormat == VIDEO_CODEC_FORMAT_XVID) && (p->vFormat.nCodecSpecificDataLen == 0))
    {
	            ...
	            CDX_LOGD("################ simple block");	 
				    ...
				    if(trackNum & 0x80)
					{
						...
						if(matroska->tracks[matroska->VideoTrackIndex]->num == trackNum)
						{
                            ...
                            //CDX_BUF_DUMP(p->vFormat.pCodecSpecificData, p->vFormat.nCodecSpecificDataLen);
							free(buf);
							goto seek_back;
						}
					}
...
cdx_int16 CdxMatroskaRead(struct CdxMkvParser *p)
{
	...
	do
	{
        if(ret!=0)
        {
        	if(p->hasIndex)
        	{
        		matroska->time_stamp_offset -= matroska->streams[matroska->VideoStreamIndex]->pts/1000;
...
int CdxMatroskaSeek(struct CdxMkvParser *p, cdx_int64  timeUs)
{
    ...
    else
    {
		...
		CDX_LOGD("%lld ms , %lld ms , FileEndPst %lld", timeUs, matroska->segment_duration, matroska->fileEndPst);
