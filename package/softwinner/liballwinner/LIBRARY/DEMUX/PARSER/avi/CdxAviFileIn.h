...
typedef struct _AVI_FILE_IN_    //从AVI_DEPACK派生出来的派生类
{
    ...
    AviChunkT         *avih;      //avi file header，需要malloc,但是长度不大。全部复制，原始文件的内容。基本不修改内容，即使错误，如avih->dwMicroSecPerFrame

    cdx_int32           nStream;    //记录AVI文件包含的音视频流总数。MainAviHeader的内容。
    AviStreamInfoT      *sInfo[MAX_STREAM]; //malloc出来的，通过get_next_stream_info(),一个SInfo对应一个LIST strl的全部内容,其下标号就是stream的index。基本上是原始文件的内容，即使认为数据明显错误，不会修改
    AudioStreamInfoT    audInfoArray[MAX_AUDIO_STREAM]; //自行算出的音频信息，用于实际。
    SubStreamInfoT      subInfoArray[MAX_SUBTITLE_STREAM];
    cdx_int8            hasIdx1;   //idx1 index table exsit,仅表示idx1表是否存在而且完整正常，只要有异常,has_idx1置0.
    cdx_int8            hasIndx;   //indx index table exsit (Open_DML AVI),只要是odml格式，就一定置1，如果置0，表示是AVI1.0格式
    ...
    cdx_int32       moviStart;     //fileoffset,after "movi", 只是RIFF AVI部分的movi_start, RIFF AVIX不考虑，所以CDX_S32足够
    cdx_int32       moviEnd;       //fileoffset,point to next byte of last_movi_byte.只是RIFF AVI部分的movi_end， RIFF AVIX不考虑， 所以CDX_S32足够
    cdx_uint32      *idx1Buf;      //old:frame_count, offset, audio_time, (audio chunk offset),
                                   //two type key frame index table,(1)3 items,(2)ref to IDX_TABLE_ITEM.
                                   //idx1_buf是建FFRRKeyframeTable的buffer。
    ...
    //cdx_int32       key_frame_index;        //current nearest key frame index, index in keyframe_table items.这个key_frame在关键帧表里的index，不是总的frame index表中的index。
    //cdx_int32       last_key_frame_index;   //last decode key frame index when ff/rr
    cdx_int32       indexInKeyfrmTbl;    //current nearest key frame index, index in keyframe_table items.这个key_frame在关键帧表里的index，不是总的frame index表中的index。
    cdx_int32       lastIndexInKeyfrmTbl;//last decode key frame index when ff/rr，也就是上一次经过的entry的在keyframe table中的序号,每次状态转换时,在SetProcMode()->AVI_IoCtrl()中初始化为-1
    ...
    AviChunkT       dataChunk;             //for store chunk data.一般分配MAX_CHUNK_BUF_SIZE大小的内存给它的buffer，而且在OpenMediaFile()中会调AVI_release_data_chunk_buf()释放掉，节省内存

    //CDX_S64       aud_chunk_counter;      //calc audio chunk count, not include current audio chunk
    //CDX_S64       aud_chunk_total_size;   //calc current audio chunk total size, not include current audio chunk size

    //顺序方式下, 在GetNextChunkInfo()中，读完一个chunk头之后,会累加统计，这样算出来的就是下一个audio chunk的起始PTS了.
    //顺序方式和index方式都用这3个变量.因为GetNextChunkInfo()中统计audio持续时间的代码是共用的。
    //index模式下index reader有自己的统计变量，但基本不用.
    //这4个变量是avi_in统计时间用的，状态转换时需重置
    cdx_int32       frameIndex;            //frame idx in all frames.,在GetNextChunkInfo()中会增加, 状态转换时setprocmode()->__reconfig_avi_read_context_readmode1()等重置，一定是从第一帧算起
    cdx_int32       frameIndex1;           //for Video stream 1
    cdx_int32       frameIndex2;           //for Video stream 2
    cdx_int64       nAudChunkCounterArray[MAX_AUDIO_STREAM];        //calc audio chunk count, not include current audio chunk，顺序模式下，不一定是从文件头开始计数的，在FRRR->PLAY之后，定位的地方开始算
    cdx_int64       nAudFrameCounterArray[MAX_AUDIO_STREAM];        //作用同上.VBR音频应该以帧为单位计算,一个chunk不一定等于一帧.这是nAudChunkCounterArray的精确替代
    cdx_int64       nAudChunkTotalSizeArray[MAX_AUDIO_STREAM];      //calc current audio chunk total size, not include current audio chunk size, 顺序模式下，不一定是从文件头开始计数的，在FRRR->PLAY之后，定位的地方开始算
    cdx_uint32      uBaseAudioPtsArray[MAX_AUDIO_STREAM];               //pts of the audio who is follow video key frame,基准时间,在parse过程中,audio_pts的计算以基准时间加上读到的数据的持续时间为准,顺序方式和index方式下都用这个变量
                                                                //for FFRR, after search key frame, reset avsync!现在又加了新的意义:随时记录当前audio chunk的PTS，以便于换音轨后能正确的设置新音轨的时间
    ...
    cdx_char        *vopPrivInf;            //vop private information, for some video format
                                            //to store private header information,e.g:H264, VC1.需要时再malloc内存，然后指针赋值给p->vFormat.pCodecExtraData
...
extern cdx_int16 AviReaderGetStreamInfo(AviFileInT *aviFile, AviStreamInfoT *sInfo, cdx_int32 index);
