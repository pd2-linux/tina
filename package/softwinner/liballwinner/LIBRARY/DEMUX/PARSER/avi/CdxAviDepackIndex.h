...
typedef struct
{
    ...
    cdx_uint32  audPtsArray[MAX_AUDIO_STREAM];//多音轨情况下，每条音轨的对应的时间戳(播放持续时间)，音轨数量参考p->hasAudio.当前的音轨下标参考p->CurItemNumInAudioStreamIndexArray
                                            //因为是基于index表的读取方式,所以记录在这个帧的PTS之后的一个audio chunk所携带的PTS.单位:ms
    cdx_int64   audChunkIndexOffsetArray[MAX_AUDIO_STREAM];     // offset of audio chunk index entry, absolute offset!那个audio frame对应的index表中的entry的绝对地址
...
} AVI_CHUNK_POSITION;           //记录的是odml indx entry所指示的chunk的信息，也可以指示idx1表的entry
...
//avi_idx1.h
extern cdx_int32 SearchNextIdx1IndexEntry(struct PsrIdx1TableReader *pReader, AVI_CHUNK_POSITION *pChunkPos);
...
extern cdx_int32 SearchNextODMLIndexEntry(ODML_SUPERINDEX_READER *pSuperReader, AVI_CHUNK_POSITION *pCkPos);
extern cdx_int16 AviBuildIdxForODMLIndexMode(CdxAviParserImplT *p);
extern cdx_int32 InitialPsrIndxTableReader(ODML_SUPERINDEX_READER *pReader, AviFileInT *aviIn, cdx_int32 streamIndex);
