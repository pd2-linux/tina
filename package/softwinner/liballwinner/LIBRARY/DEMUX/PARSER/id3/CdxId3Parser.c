...
static void	ID3PsrReadNunicode(cdx_int32 *FrameLen,cdx_int8** Frame, Id3ParserImplS *id3,int frameTLen)
{
	cdx_int8 data;
	cdx_int32 i;
...
static void	ID3PsrReadunicode(cdx_int32 *FrameLen,cdx_int8** Frame, Id3ParserImplS *id3,int frameTLen)
{
...
static int Id3Init(CdxParserT *id3_impl)
{
    ...
    if(strncmp(impl->keyinfo, "file://", 7) == 0 || strncmp(impl->keyinfo, "fd://", 5) == 0)
    {
        ...
        ret = sprintf(impl->newurl, "fd://%d?offset=%lld&length=%lld", tmpFd, impl->fdoffset+impl->file_offset, impl->file_size - impl->file_offset);
    }
...
static cdx_int32 __Id3ParserControl(CdxParserT *parser, cdx_int32 cmd, void *param)
{
    ...
    switch (cmd)
    {
    CDX_LOGD("******in id3parser,cmd = %d",cmd);
    ...
    case CDX_PSR_CMD_SET_FORCESTOP:
		impl->forceStop = 1;
		CDX_LOGD("******in id3parser,CDX_PSR_CMD_SET_FORCESTOP*********");
		if(impl->childStream){
