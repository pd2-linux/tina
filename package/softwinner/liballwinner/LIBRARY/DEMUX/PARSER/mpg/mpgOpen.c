...
cdx_uint8 CheckFirstPack(CdxMpgParserT *MpgParser, cdx_uint8 *buf)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if(MpgTimeMpeg1ReadPackSCR(pOrgBuf, &mMpgParserCxt->nStartScrLow,&mMpgParserCxt->nStartScrHigh) == 0)
		        {
			     mMpgParserCxt->nBaseSCR = (mMpgParserCxt->nStartScrLow>>1) | (mMpgParserCxt->nStartScrHigh<<31);
                 return 1;
		        }   
            }
            else
            {
               if(MpgTimeMpeg2ReadPackSCR(pOrgBuf, &mMpgParserCxt->nStartScrLow,&mMpgParserCxt->nStartScrHigh,NULL) == 0)
		       {
			       mMpgParserCxt->nBaseSCR = (mMpgParserCxt->nStartScrLow>>1) | (mMpgParserCxt->nStartScrHigh<<31);
                   return 1;
		       }
	...
	while (nEsMapLength >= 4)
	{
		...
		switch(nType)
		{
		   ...
		   case CDX_STREAM_TYPE_AUDIO_MPEG2:
			   // MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_MP2;
			    CDX_LOGV("need process audio info.\n");
				break;
		   case CDX_STREAM_TYPE_AUDIO_AAC:
			   // MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_MPEG_AAC_LC;
			    CDX_LOGV("need process audio info.\n");
				break;
		   case CDX_STREAM_TYPE_CDX_AUDIO_AC3:
			  // MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_AC3;
			   CDX_LOGV("need process audio info.\n");
               break;
		   default: break;
		}
...
cdx_uint8 CheckVideoIdNo1172(CdxMpgParserT *MpgParser, cdx_uint8 *buf, cdx_uint32 off, cdx_uint32 pes_len)
{
	...
	{            
		if(buf[off]==0x00 && buf[off+1]==0x00 && buf[off+2]==0x01 && buf[off+3]==0xb3) //check sequence header in 13818-2
		{   
...
cdx_uint8 CheckVideoId1172(CdxMpgParserT *MpgParser, cdx_uint8 *buf, cdx_uint32 off, cdx_uint32 pes_len)
{
	...
	while (pes_len)
	{
		if(buf[off]==0x00 && buf[off+1]==0x00 && buf[off+2]==0x01 && buf[off+3]==0xb3) //check 11172-2 sequence header
		{   
            mMpgParserCxt->bRecordSeqInfFlag = 1;
            memcpy(mMpgParserCxt->nSequenceBuf, buf+off, MPG_SEQUENCE_LEN);
			off += 4;
...
cdx_uint8 CheckAudioType(CdxMpgParserT *MpgParser, cdx_uint8 *buf, cdx_uint32 offset)
{
	...
	if(code>=0x80 && code <=0x87)
	{
		...
		{
            if((MpgParser->mAudioFormatTArry[i].eCodecFormat==AUDIO_CODEC_FORMAT_AC3)&&(mMpgParserCxt->bAudioStreamIdCode[i]==code))
            {
                break;
            }
        }
		...
		{
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_CODEC_FORMAT_AC3;
		...
		||(code>=0x98 && code <=0x9f))
	{
		...
		for(i=0; i<MpgParser->nhasAudioNum; i++)
		{
            if((MpgParser->mAudioFormatTArry[i].eCodecFormat==AUDIO_CODEC_FORMAT_DTS)&&(mMpgParserCxt->bAudioStreamIdCode[i]==code))
            {
                break;
            }
        }
        if(i == MpgParser->nhasAudioNum)
		{
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_CODEC_FORMAT_DTS;
            mMpgParserCxt->nAudioIdArray[MpgParser->nhasAudioNum] = MPG_PRIVATE_STREAM_1;
            mMpgParserCxt->bAudioStreamIdCode[MpgParser->nhasAudioNum] = code;
            MpgParser->nhasAudioNum += 1;
        }
	...
	else if(code >=0xa0 && code <=0xaf)
	{
		...
		for(i=0; i<MpgParser->nhasAudioNum; i++)
		{
            if((MpgParser->mAudioFormatTArry[i].eCodecFormat==AUDIO_CODEC_FORMAT_PCM)&&(mMpgParserCxt->bAudioStreamIdCode[i]==code))
            {
                break;
            }
        }
        if(i == MpgParser->nhasAudioNum)
		{
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_CODEC_FORMAT_PCM;
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].nBitsPerSample = quantWordLen[(buf[off+5]>>6)&0x03];
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].nSampleRate = audioSampleFreq[(buf[off+5]>>4)&0x03];
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].nChannelNum = (buf[off+5]&0x07)+1;
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eSubCodecFormat = WAVE_FORMAT_PCM | ABS_EDIAN_FLAG_BIG;
            mMpgParserCxt->nAudioIdArray[MpgParser->nhasAudioNum] = MPG_PRIVATE_STREAM_1;
            mMpgParserCxt->bAudioStreamIdCode[MpgParser->nhasAudioNum] = code;
            MpgParser->nhasAudioNum += 1;
        }
	...
	else if(code >=0xb0 && code <=0xbf)
	{
		...
		for(i=0; i<MpgParser->nhasAudioNum; i++)
		{
            if((MpgParser->mAudioFormatTArry[i].eCodecFormat==AUDIO_CODEC_FORMAT_MLP)&&(mMpgParserCxt->bAudioStreamIdCode[i]==code))
            {
                break;
            }
        }
        if(i == MpgParser->nhasAudioNum)
		{
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_CODEC_FORMAT_MLP;
            mMpgParserCxt->nAudioIdArray[MpgParser->nhasAudioNum] = MPG_PRIVATE_STREAM_1;
            mMpgParserCxt->bAudioStreamIdCode[MpgParser->nhasAudioNum] = code;
            MpgParser->nhasAudioNum += 1;
        }
...
int CheckPes(unsigned char *MpgParser, unsigned char *pEndPtr)
{
    ...
    int pes2 = ((MpgParser[3] & 0xC0) == 0x80) && ((MpgParser[4] & 0xC0) != 0x40)    //PTS_DTS_flag value '01' is forbidden
		&& ((MpgParser[4] & 0xC0) == 0x00 || ((MpgParser[4]&0xC0)>>2) == (MpgParser[6]&0xF0));
    ...
    {
        pes1= MpgParser[0]&MpgParser[2]&MpgParser[4]&MpgParser[5]&MpgParser[7]&MpgParser[9]&1; //check the marker_bit
        MpgParser+=10;
    }else
        pes1 = (*MpgParser == 0x0F);
...
#if 0
cdx_uint8 *check_pack_start_code(cdx_uint8 *buf)
{
    if(buf[0] != 0x00 || buf[1] != 0x00 || buf[2] != 0x01 || buf[3] != 0xba)
    {
        return NULL;
...
cdx_uint8 *find_next_pack(cdx_uint8 *buf, cdx_uint8 *buf_end)
{
    unsigned int start_code = 0x000001ba;
    unsigned int code;

    code = (unsigned int)buf[0];
    code <<= 8;
    code |= (unsigned int)buf[1];
    code <<= 8;
    code |= (unsigned int)buf[2];
    code <<= 8;
    code |= (unsigned int)buf[3];
    buf += 4;
    while(buf < buf_end)
    {
        unsigned int t1, t2;
        t1 = code<<8;
        t2 = (unsigned int)buf[0];
        if(code == start_code)
        {
            if((buf[0]>>4) == 0x02)    //MPEG1
            {
                if( buf+7 >= buf_end)
                {
                    break;
...
{   
   ...
   if(mMpgParserCxt->bIsISO11172Flag)
   {
      curPtr = MpgReadProcessIsISO11172(MpgParser, curPtr, &result, &packetLen, &ptsLow, &ptsHigh);
   }
   else
   {
     curPtr = MpgReadProcessNonISO11172(MpgParser, curPtr, &result, &packetLen, &ptsLow, &ptsHigh);
   }
   ...
   {   
       ...
       if((adtsFlag[0]==0) && (adtsFlag[1]==0) &&  (adtsFlag[2]==0) &&(adtsFlag[3]==0))
            ...
            MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_CODEC_FORMAT_MPEG_AAC_LC;
   }
}



cdx_uint8 *ProcessStartCode(CdxMpgParserT *MpgParser,cdx_uint8 *checkBufPtr,cdx_uint32 startCode,cdx_uint32 retDatalen,cdx_uint32 *packInf, cdx_uint32 *calPackNum)
{   
    MpgParserContextT *mMpgParserCxt = NULL;
    ...
    //   cdx_uint32 packInf[4]      = {1,1,0,0};        //0: isfstSystemFlag,1:isfstPackFlag,2:fstSysPackPos, 3, lastPackPos
    ...
    { 
       if(packInf[0] == 1)
       {
          packInf[3] = CdxStreamTell(mMpgParserCxt->pStreamT) - retDatalen + (checkBufPtr-mMpgParserCxt->mDataChunkT.pStartAddr) + 4;
       }
       if(packInf[1] == 1)
       {
    ...
    else if((startCode == MPG_PADDING_STREAM) || (startCode == MPG_PRIVATE_STREAM_2))       // 0x000001BE 0x000001BF
    {
       bufOffset = (checkBufPtr[4]<<8) | checkBufPtr[5];
	   checkBufPtr += (bufOffset + 6);
    }
    else        // video pack, audio pack, subtitle pack
    {
       ...
       {  
          ...
          {
            if(CheckVideoId(MpgParser, checkBufPtr, 3, startCode) == 1)           // find the sequence header
            {
                calPackNum[vidPack]++; 
            }
          }
       ...
       {  
          ...
          if(i == MpgParser->nhasAudioNum)
          {
                MpgParser->mAudioFormatTArry[MpgParser->nhasAudioNum].eCodecFormat = AUDIO_CODEC_FORMAT_MP2;
...
__read_new_data:
    ...
    {  
       ...
       {  
          checkBufPtr = ProcessStartCode(MpgParser,checkBufPtr,startCode,retDatalen,packInf,calPackNum);
       }
    }
 __cal_parse_result:
    ...
    {  
        ...
        {
            memcpy(mMpgParserCxt->mDataChunkT.pStartAddr, mMpgParserCxt->mDataChunkT.pStartAddr+retDatalen-256, 256);
    ...
    if(mMpgParserCxt->bIsNetworkStream == 1 && mMpgParserCxt->bSeekDisableFlag==1)
    {
        ...
        mMpgParserCxt->mDataChunkT.pEndPtr            = mMpgParserCxt->mDataChunkT.pStartAddr+MAX_CHUNK_BUF_SIZE;
