...
#if 0
void MPG_compute_file_timelen(CdxMpgParserT *MpgParser)
{
   MpgParserContextT *mMpgParserCxt;
   mMpgParserCxt = (MpgParserContextT *)MpgParser->pMpgParserContext;

   if((mMpgParserCxt->nStartScrHigh == mMpgParserCxt->nEndScrHigh) && (mMpgParserCxt->nEndScrLow < mMpgParserCxt->nStartScrLow))
   {
	   mMpgParserCxt->nStartScrLow = 0;
   }
   if((mMpgParserCxt->nStartScrHigh == mMpgParserCxt->nEndScrHigh) && (mMpgParserCxt->nEndScrLow > mMpgParserCxt->nStartScrLow))
	{
	   mMpgParserCxt->nFileTimeLength = (mMpgParserCxt->nEndScrLow - mMpgParserCxt->nStartScrLow);
	   mMpgParserCxt->nFileTimeLength /= 90;
	}
	else if((mMpgParserCxt->nStartScrHigh < mMpgParserCxt->nEndScrHigh)  && (mMpgParserCxt->nEndScrLow < mMpgParserCxt->nStartScrLow))
	{
		mMpgParserCxt->nFileTimeLength = (0xffffffff - mMpgParserCxt->nStartScrLow) + 1 + mMpgParserCxt->nEndScrLow;
		mMpgParserCxt->nFileTimeLength /= 90;
	}
}
...
cdx_int16 MpgTimeCheckPsPts(CdxMpgParserT *MpgParser)
{
    ...
    {    
		...
		readLen = CdxStreamRead(mMpgParserCxt->pStreamT,mMpgParserCxt->mDataChunkT.pStartAddr,MAX_CHUNK_BUF_SIZE);
		readDataLen += readLen;
...
find_pack_start_code0:
        if(MpgParser->bForceReturnFlag == 1)
           return FILE_PARSER_OPEN_FILE_FAIL;
		buf = MpgOpenFindPackStartReverse(mMpgParserCxt->mDataChunkT.pEndPtr ,mMpgParserCxt->mDataChunkT.pStartAddr);
...
cdx_int16 MpgTimeCheckPsNetWorkStreamPts(CdxMpgParserT *MpgParser, u32 startPos)
{
    ...
    while(1)
    {
		...
		mMpgParserCxt->mDataChunkT.pEndPtr = mMpgParserCxt->mDataChunkT.pStartAddr + readLen+nReadLen;
		checkStartAddr = mMpgParserCxt->mDataChunkT.pStartAddr;
find_pack_start_code1:
        if(MpgParser->bForceReturnFlag == 1)
        {
	    ...
	    memcpy(mMpgParserCxt->mDataChunkT.pStartAddr, mMpgParserCxt->mDataChunkT.pStartAddr+MAX_CHUNK_BUF_SIZE-4, 4);
...
cdx_int16 VobSearchNextNvPack(CdxMpgParserT *MpgParser, cdx_int64 *startFpPos, cdx_uint8 *scrChangedFlag)
{
    MpgParserContextT *mMpgParserCxt = NULL;
    cdx_uint8 *buf   	   = NULL;
    cdx_uint8 *curPtr      = NULL;
...
cdx_int16 VobCheckEndPts(CdxMpgParserT *MpgParser, cdx_int64 curFpPos, cdx_int64 endFpPos, cdx_int64 *orgEndPTM, cdx_uint8 calLastPtsFlag)
{
    MpgParserContextT *mMpgParserCxt=NULL;
    ...
    CdxStreamSeek(mMpgParserCxt->pStreamT, curFpPos, STREAM_SEEK_SET);
    redLen  = CdxStreamRead(mMpgParserCxt->pStreamT,mMpgParserCxt->mDataChunkT.pStartAddr, VOB_VIDEO_LB_LEN);
...
cdx_int16  VobCheckPsPts(CdxMpgParserT *MpgParser, cdx_int64 fstSysPos)
{
  ...
  {    
      ...
      {
        next32code = ((mMpgParserCxt->mDataChunkT.pStartAddr[0]<<24) | (mMpgParserCxt->mDataChunkT.pStartAddr[1]<<16)|(mMpgParserCxt->mDataChunkT.pStartAddr[2]<<8)|mMpgParserCxt->mDataChunkT.pStartAddr[3]);
...
__cal_new_fp_pos:   
     ...
     {  
       ...
       if(curFpPos >= mMpgParserCxt->nFileLength)
       {
         ...
         for(j=i+1; j<20; j++)
         {
            if((mMpgParserCxt->nRecordVobuPosArray[j]!=0x00ffffff)&&(mMpgParserCxt->nRecordVobuPosArray[j]!=0))
            {
         ...
         if((j==20) || (mMpgParserCxt->nRecordVobuPosArray[j] == mMpgParserCxt->nRecordVobuPosArray[19]))
         {
   ...
   if((curVobId == 0) &&(mMpgParserCxt->mFileTitleIfoT.nVobEndPosArray[curVobId] == mMpgParserCxt->mFileTitleIfoT.nVobStartPosArray[curVobId]))
   {
      mMpgParserCxt->bHasNvpackFlag = CDX_FALSE;
      mMpgParserCxt->nFileTimeLength = 0;
   ...
   else
   {
      ...
      for(curVobId=2; curVobId<=mMpgParserCxt->nFileTitleNs; curVobId++)
      {
            if(mMpgParserCxt->mFileTitleIfoT.bVobScrChangedFlagArray[curVobId-2]==1)
            {
                ...
                mMpgParserCxt->mFileTitleIfoT.nVobBaseTimeArray[curVobId-1] = lastBaseTime + mMpgParserCxt->nBasePts;
            }
            else
            {    
            ...
            mMpgParserCxt->mFileTitleIfoT.nVobTimeLenArray[curVobId] = lastBaseTime + vobfileTimeLen[curVobId-1];
      }
      
      mMpgParserCxt->nFileTimeLength = mMpgParserCxt->mFileTitleIfoT.nVobTimeLenArray[mMpgParserCxt->nFileTitleNs];
