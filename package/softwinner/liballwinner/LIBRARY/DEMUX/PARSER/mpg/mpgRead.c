...
{   
    ...
    else if((cur_id & 0xe0) == MPG_AUDIO_ID)
	{
		if(cur_id != mMpgParserCxt->nAudioId)
			return NULL;
	}
	else if(mMpgParserCxt->nAudioId==MPG_PRIVATE_STREAM_1 && cur_id==mMpgParserCxt->nAudioId)
	{
		code = *cur_packet_ptr;
        if(code != mMpgParserCxt->nAudioStreamId)
        {
            return NULL;
        }
        #if 0
		if(!mMpgParserCxt->bAudioIdFlag)
		{
			switch(MpgParser->mAudioFormatT.eCodecFormat)
			{
			case AUDIO_AC3:
				if((code>= 0x80 && code<=0x87) ||(code>=0xc0 && code <=0xcf))
				{
					mMpgParserCxt->audio_id_num = code;
					mMpgParserCxt->bAudioIdFlag = CDX_TRUE;
				}
				break;
			case AUDIO_DTS:
				if((code>=0x88 && code <=0x8f) ||(code>=0x98 && code <=0x9f))
				{
					mMpgParserCxt->audio_id_num = code;
					mMpgParserCxt->bAudioIdFlag = CDX_TRUE;
				}
		...
		if(!mMpgParserCxt->bAudioIdFlag || (mMpgParserCxt->bAudioIdFlag && (code != mMpgParserCxt->audio_id_num)))
		{
			return NULL;
		}
		#endif
...
cdx_uint8 ProcessVideoData(CdxMpgParserT *MpgParser,  MpgVideoFrmItemT* mVidFrmItemT, cdx_uint8*curPtr, cdx_uint32 packetLen)
{   
    cdx_uint32 copySize1 = 0;
    cdx_uint32 copySize2 = 0;
    
    if((MpgParser->mMpgVidFrmInfT.pCurVidFrmDataPtr+packetLen)>= MpgParser->mMpgVidFrmInfT.pVidFrmDataBufEnd)
    {
        copySize1 = MpgParser->mMpgVidFrmInfT.pVidFrmDataBufEnd - MpgParser->mMpgVidFrmInfT.pCurVidFrmDataPtr;
...
cdx_uint8 MpgReadJudgeNextDataType(CdxMpgParserT *MpgParser)
{
   ...
   {  
      ...
      {
        if((nNext32bits == mMpgParserCxt->nAudioId) || (nNext32bits == mMpgParserCxt->nVideoId) || (nNext32bits == 0x01BD))
        {
...
cdx_uint8 MpgReadParserReadData(CdxMpgParserT *MpgParser)
{
   ...
   {   
       nSize = CdxStreamRead(mMpgParserCxt->pStreamT,mMpgParserCxt->mDataChunkT.pStartAddr,MAX_CHUNK_BUF_SIZE);
...
{   
          ...
          if(dvdInfo->titleIfoFlag == 1)
            ...
            {   
                ...
                {   
                    ...
                    flag2 = (cur_id==MPG_PRIVATE_STREAM_1)&&(code==mMpgParserCxt->bAudioStreamIdCode[i]);
                    if((flag1==1) ||(flag2==1))
                    {   
                        mMpgParserCxt->nSendAudioIndex = i;
                        break;
    ...
    {   
        ...
        // CDX_LOGV("*************here3:cur_id=%x, code=%x,mMpgParserCxt->mDataChunkT.nSubId=%x, flag1=%x, flag2=%x\n",
...
{   
	...
	curBuf = MpgOpenFindNextStartCode(MpgParser,mMpgParserCxt->mDataChunkT.pReadPtr, mMpgParserCxt->mDataChunkT.pEndPtr);
 ...
 {   
	 ...
	 *result = -1;
     ...
     if(flag1 && (flag2 || flag3))
	 {
		 ...
		 {   
			 curBuf = MpgOpenFindNextStartCode(MpgParser, cur_ptr+6,mMpgParserCxt->mDataChunkT.pEndPtr);
...
cdx_int16 MpgReadAddPacketIntoArray(CdxMpgParserT *MpgParser, cdx_uint32 cur_id, cdx_uint8 sub_stream_id, cdx_uint8 *curPtr,
 ...
 {   
	 ...
	 if(cur_id==mMpgParserCxt->nVideoId && mMpgParserCxt->bSwitchScrOverFlag==CDX_TRUE)
	 {
		 ...
		 mMpgParserCxt->nBaseTime = mMpgParserCxt->nPreVideoMaxPts + ((1000000UL + (MpgParser->mVideoFormatT.nFrameRate>>1))/MpgParser->mVideoFormatT.nFrameRate);
...
search_pic_start_code:
    ...
    mVidFrmItemT = &(MpgParser->mMpgVidFrmInfT.mVidFrmItemT[MpgParser->mMpgVidFrmInfT.nVidFrmRcdItemIdx]);
    if(i == packetLen)
    {
