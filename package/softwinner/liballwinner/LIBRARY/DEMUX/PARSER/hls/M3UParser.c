		...
		*equalPos = '\0';
        ...
        if (!strcasecmp("type", key)) 
		{
			...
			else 
            {
				...
				goto _err;/*有些type我们不识别，如CLOSED-CAPTIONS，此时不能认为是出错，否则会导致整个m3u8parser出错退出*/
    ...
    while (offset < line->mSize) 
	{
		...
		*equalPos = '\0';
                ...
                || !strcmp("subtitles", key)) 
		{
			...
			/*例如，第一次循环时groupID被写入meta，但第二次循环中出错，此时destoryAString(groupID);和destoryPlaylist重复释放内存*/
        }
    }
...
_err:
	...
	destoryAString(groupID);
	/*此时可能有一些groupID已经写入meta，该处并未将其释放，因为groupID处于while中。没有关系，返回错误，destoryPlaylist会将其释放*/
    return err;
}
...
status_t M3u8Parse(const void *_data, cdx_uint32 size, Playlist **P, const char *baseURI) 
{
	*P = NULL;
	...
	int mIsVariantPlaylist = -1;/*设置这个变量的原因是playList->mIsVariantPlaylist是bool，在未明确判断之前已经有值0*/
    ...
    {   
		...
		else
        {
            ...
            memcpy(line.mData, &data[offset], offsetData - offset);    /*从data[offset]读到data[offsetData-1]构成一行*/
        ...
        else
        {
            if (startsWith(line.mData,"#EXT-X-TARGETDURATION")) 
            {
                ...
                {
					CDX_LOGE("ERROR_MALFORMED");
					err = ERROR_MALFORMED;/*这些tag不会出现在master playlist 中，而只出现在media playlisy中*/
					goto _err;
                }
