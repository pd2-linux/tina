...
typedef struct CdxFlvParserImplS
{
    ...
    cdx_int32       h265_vps_sps_pps_state;// 3: read vps 2: read sps 1: read pps , 0: back seek , -1: invalid    
...
cdx_int32 CheckTagValid(FlvTagsT *pTagHead, cdx_uint32 nTagSize)
{
    ...
    {
        CDX_LOGE("CHECKTag Invalid! last_tag_size[%d]!=s->tag.dataSize[%d] + 11", nTagSize, uDataSize);
        return -1;
    }
}
...
cdx_int32 GetTotalTimeFromHead(struct VdecFlvIn *s)
{
    ...
    if(0 == tmpret)
    {
        tmpret = CdxStreamSeek(s->fp, (cdx_int32)(EndianConvert(curTag->dataSize,3)-1), STREAM_SEEK_CUR);
        if(tmpret)
        {
			...
			case Keyframes:
          	{
				...
				for(keyframes_counter = 0; keyframes_counter < 2; keyframes_counter++)
				{
					...
					if(strcmp(buffer, "times"))//case position index
					{
						...
						if((s->keyFrameIndex.keyFramePosIdx = (cdx_int64 *)malloc(sizeof(cdx_int64)*length)) == NULL)
						{
						...
						{
							if(CdxStreamRead(s->fp, &type, 1) != 1 || type != CDX_AMF_DATA_TYPE_NUMBER)
							{
							...
							if(CdxStreamRead(s->fp, &s->keyFrameIndex.keyFramePosIdx[index], 8) != 8)
							{
								CDX_LOGE("read pos index failed.");
								return -1;
							}
							s->keyFrameIndex.keyFramePosIdx[index] = EndianConvertInt64(s->keyFrameIndex.keyFramePosIdx[index], 8);
							temdouble = int2dbl(s->keyFrameIndex.keyFramePosIdx[index]);
							s->keyFrameIndex.keyFramePosIdx[index] = (cdx_int64)temdouble;
							CDX_LOGV("s->keyFrameIndex.keyFramePosIdx[0]: %llx",s->keyFrameIndex.keyFramePosIdx[index]);
						}
					}
					else//times
					{
						if((s->keyFrameIndex.keyFramePtsIdx = (cdx_int64 *)malloc(sizeof(cdx_int64)*length)) == NULL)
						{
						...
						for(index = 0; index < length; index++)
						{
							...
							s->keyFrameIndex.keyFramePtsIdx[index] = EndianConvertInt64(s->keyFrameIndex.keyFramePtsIdx[index], 8);
...
/**************************************************
  FLV_reader_open_file()
    ...
    -2   wrong format
**************************************************/
cdx_int16 FlvReaderOpenFile(CdxFlvParserImplT *impl)//struct FILE_PARSER *p,CedarXDataSourceDesc *file_name
{
    cdx_int32               result;
    struct VdecFlvIn      *s;
    FlvFileHeaderT         *t;
    ...
    do
    {
        ...
        if(s->tag.tagType == 8 && !check_audio && t->typeFlagAudio)
        {
            ...
            {
                result = CdxStreamRead(s->fp, s->dataChunk.buffer, (cdx_uint32)(s->tag.dataSize - 1));
        ...
        else if(s->tag.tagType == 9 && !check_video && t->typeFlagVideo)
        {
            ...
            if(s->tag.dataSize >= sizeof(FlvVideoTagsT))
            {
                ...
                if(tempBuf == NULL)
                {
                    CDX_LOGE("malloc failed. size=%u, pos:%lld", s->tag.dataSize-(unsigned int)(sizeof(FlvVideoTagsT)), CdxStreamTell(s->fp));
                    return -1;
                }
                ...
                if(s->seekAble != 1)
                {
                    ...
                    }else
                    {
                        ...
                        memcpy(videoTotalData + sizeof(FlvVideoTagsT), tempBuf, s->tag.dataSize-sizeof(FlvVideoTagsT));
                    }
					free(tempBuf);
					tempBuf = NULL;
                    if (s->videoTag.codecID == 7)
                    {
                        {//--------------------------------------------------------------
                            if(!impl->tempBuf)
                            {
                                impl->tempBufLen = s->tag.dataSize -1-4; //* if AVC, skip first 4 bytes, AVCPacketType(1B), CompositionTime(3B)
                ...
                #if 0
                result = CdxStreamSeek(s->fp, (cdx_int32)(s->tag.dataSize-sizeof(FlvVideoTagsT)), STREAM_SEEK_CUR);
                if(result)
                {
                    CDX_LOGW("Skip first video tag data failed!");
                    break;
    ...
    if(t->typeFlagVideo)
    {
        if(s->videoTag.codecID == 2)//Sorenson H.263
        {
            ...
            switch(c)
            {
                case 0:
                {
                    impl->vFormat.nWidth = ((s->videoTag.sizeData0)<<1) | ((s->videoTag.sizeData1>>7)&1);
                    impl->vFormat.nHeight = ((s->videoTag.sizeData1&0x7f)<<1) | ((s->videoTag.sizeData2>>7)&1);
                    break;
                }
                case 1:
                {
                    impl->vFormat.nWidth = ((s->videoTag.sizeData0)<<9) | ((s->videoTag.sizeData1<<1)&0x1fe) | ((s->videoTag.sizeData2>>7)&1);
                    impl->vFormat.nHeight = ((s->videoTag.sizeData2&0x7f)<<9) | ((s->videoTag.sizeData3<<1)&0x1fe) | ((s->videoTag.sizeData4>>7)&1);
                    break;
                }
                case 2:
                {
	                ...
	                {//--------------------------------------------------------------
	                    if(!impl->tempBuf)
	                    {
	                        ...
	                        if(impl->tempBufLen > 0)
	                        {
	                            ...
	                            memcpy(impl->tempBuf, (char *)s->dataChunk.buffer+1+4, impl->tempBufLen);
	                        }
	                    }
	                }
	                if (s->dataChunk.buffer[1] == 1)//AVC NALU
    ...
    {
        if(impl->vFormat.eCodecFormat == VIDEO_CODEC_FORMAT_UNKNOWN || impl->aFormat.eCodecFormat == AUDIO_CODEC_FORMAT_UNKNOWN)
        {
            impl->hasVideo = 0;
            impl->hasAudio = 0;
        }
...
*/
	   if(s->tag.dataSize == 0 && s->tag.streamID == 0 && 
		...
		{
			result = CdxStreamSeek(s->fp, s->tag.dataSize - 0x1, STREAM_SEEK_CUR);//Skip MetaData Tag
			if(result < 0)
			{
                CDX_LOGE("Seek failed.");
                return -1;
        ...
        if(s->tag.tagType == 8 && impl->hasAudio == 1)
        {
            ...
            if(impl->aFormat.eCodecFormat == AUDIO_CODEC_FORMAT_MPEG_AAC_LC)
            {
                ...
                else
                {
                    ...
                    result = CdxStreamSeek(s->fp, (cdx_int32)(s->tag.dataSize-1), STREAM_SEEK_CUR); //read to a temp buf maybe better, but live stream?
                    if(result < 0)
                    {
...
HEVC_vps_sps_pps_state: //for Breakpoint play
    ...
    switch(impl->h265_vps_sps_pps_state)
    {
        ...
        {
            result = CdxStreamSeek(tmpFlvIn->fp, (cdx_int64)tmpFlvIn->header.dataOffset, STREAM_SEEK_SET);
            if(result)
            {
...
static void FillHeader(cdx_uint8 *buf0, cdx_uint8 *buf1, cdx_int32 size, cdx_int32 *size0, cdx_int32 flag)
{
    cdx_int32 i, j, k = 0;
        ...
        while(totSize)
        {
            ...
            else
            {
                ...
                *(pktBuf0+7) = 1;
                ...
                pktBuf0 += (size-2+10);
                //CDX_LOGV("xxx  pkt_buf0[8-11]%x,%x,%x,%x,%x",pktBuf0[8],pktBuf0[9],pktBuf0[10],pktBuf0[11],pktBuf0[12]);
...
static cdx_int32 __CdxFlvParserRead(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    if(impl->vFormat.eCodecFormat != VIDEO_CODEC_FORMAT_H264 || impl->curChunkInfo.nChkType != CDX_TagVideo) //  audio, subtitle and non h264 video.
    {
        cdx_uint8 pktTemp[10];
        pkt->length = impl->curChunkInfo.nTotSize;

        if(impl->vFormat.eCodecFormat == VIDEO_CODEC_FORMAT_H265 && impl->curChunkInfo.nChkType == CDX_TagVideo)
        {
            ...
            if(tmpFlvIn->videoTag.codecID == 0x09) //aiqiyi
            {
               ...
               if(impl->curChunkInfo.nTotSize <= pktSize0) 
               {
                    ...
                    if(CdxStreamRead(stream, pktBuf0, impl->curChunkInfo.nTotSize-4) != impl->curChunkInfo.nTotSize-4)
                    {
                         CDX_LOGW("xxx read data fail.");
                         return -1;
                    }
               ...
               else if(impl->curChunkInfo.nTotSize > pktSize0 && impl->curChunkInfo.nTotSize < pktSize0 + 4 )
               {
                    ...
                    memcpy(pktBuf0+ impl->curChunkInfo.nTotSize -4, pktTemp, pktSize0+4- impl->curChunkInfo.nTotSize);
                    memcpy(pktBuf1,pktTemp + pktSize0+4- impl->curChunkInfo.nTotSize, impl->curChunkInfo.nTotSize - pktSize0);
        ...
        else if(impl->vFormat.eCodecFormat == VIDEO_CODEC_FORMAT_VP6 && impl->curChunkInfo.nChkType == CDX_TagVideo)
        {
            ...
            if(impl->curChunkInfo.nTotSize <= pktSize0)
            {
                if(CdxStreamRead(stream, pktBuf0, impl->curChunkInfo.nTotSize) != impl->curChunkInfo.nTotSize)
                {
...
static cdx_int32 __CdxFlvParserSeekTo(CdxParserT *parser, cdx_int64 timeUs)
{
    ...
    if(s->keyFrameIndex.hasKeyFrameIdx == 1)
    {
        ...
        if(0/*s->keyFrameIndex.keyFramePtsIdx[mid] == 0*/) //FLV020_IFTlhxwwkKE_320x320_0.007fps_Sore....flv. just 1 video frame, a|v|aaaaaaaaaaa...
        {
        ...
        switch(mode)
        {
            ...
            case 2:
            {
                ...
                while(1)
                {
                    ...
                    if((s->tag.tagType == 9) && ((s->tag.codecType & 0xf0) == 0x10))
                    {
                        ...
                        {
                            //* check which one is close to seekTime,  this keyframe's pts or last keyframe.
                            if(FLV_ABS(seekTime - s->tag.timeStamp) >= FLV_ABS(s->lastKeyFrmPts - seekTime)) //* seekTime is closer to lastKeyframe pts
                            {
                            ...
                            else
                            {
                                ...
                                result = CdxStreamSeek(s->fp, (cdx_int32)(0-sizeof(FlvTagsT)), STREAM_SEEK_CUR);
                                if(result < 0)
                                {
                        ...
                        CDX_LOGV("xxx fb s->tag.timeStamp(%u) > seekTime(%lld), continue find keyframe...", s->tag.timeStamp, seekTime);
                    }

                    //seek to previous tag header
                    result = CdxStreamSeek(s->fp, (cdx_int32)(0-(sizeof(FlvTagsT)+s->tag.preTagsize+4)), STREAM_SEEK_CUR);
                    if(result < 0)
                    {
                        CDX_LOGW("Seek back to the previous tag header failed. s->tag.preTagsize=%u", s->tag.preTagsize);
                        //impl->mErrno = PSR_UNKNOWN_ERR;
                        //ret = -1;
                        //goto __exit;
            ...
            default:
            {
                ...
                {                
                    ...
                    len = CdxStreamRead(s->fp, &s->tag, sizeof(FlvTagsT));
                    if(len != sizeof(FlvTagsT))//may not found keyframe even read to file end, seek to last keyframe.
                    {                    
                        result = CdxStreamSeek(s->fp, s->lastKeyFrmPos, STREAM_SEEK_SET);
                        if(result < 0)
                        {
                    ...
                    if((s->tag.tagType == 9) && ((s->tag.codecType&0xf0) == 0x10))
                    {
                        ...
                        {                        
                            //* check which one is close to seekTime, this keyframe's pts or last keyframe.
                            if(FLV_ABS(s->tag.timeStamp - seekTime) >= FLV_ABS(seekTime - s->lastKeyFrmPts)) //* seekTime is closer to lastKeyframe pts
                            {
                                result = CdxStreamSeek(s->fp, s->lastKeyFrmPos, STREAM_SEEK_SET);
                                if(result < 0)
                                {
                        ...
                        s->lastKeyFrmPts = s->tag.timeStamp;
                        s->lastKeyFrmPos = CdxStreamTell(s->fp) - sizeof(FlvTagsT);
                        CDX_LOGV("xxx ff s->tag.timeStamp(%u) < seekTime(%lld), continue find keyframe...", s->tag.timeStamp, seekTime);
                    }
...
static cdx_uint32 __CdxFlvParserAttribute(CdxParserT *parser) /*return falgs define as open's falgs*/
{//not use yet..
    CdxFlvParserImplT *impl;
