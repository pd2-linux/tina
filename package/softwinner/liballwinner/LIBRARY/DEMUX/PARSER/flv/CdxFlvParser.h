...
typedef struct
{
    ...
    unsigned soundFormat            :   4;        //0: uncompresed, 1: ADPCM, 2: MP3, 5: Nellymoser 8khz mono, 6: Nellymoser
...
typedef struct
{
    unsigned codecID                :   4;        //2: Sorensson H.263, 3: screen video, 4: On2 VP6, 5: On2 VP6 wiht alpha channel, 6: Screen video version 2
...
struct VdecFlvIn
{
    ...
    cdx_uint32        lastKeyFrmPos; // the previous keyframe pos, for seekTo while has no keyframe index when not found keyframe.
    cdx_uint32        lastFrmPts;    // the previous frame pts
    cdx_uint32        lastFrmPos;    // the previous frame pos, for seekTo while has no keyframe index. 
