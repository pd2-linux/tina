...
typedef struct AVCodecContext 
{
    ...
    CDX_S32			  channels;
	CDX_U32			  codecTag;     // the tag of the codec in the file, is may be unused( like: "mp4v", "xvid", ...) 
...
typedef struct MOVStreamContext 
{
    ...
    CDX_S32         sample_duration;   //the first sample duration of stts atom,  for calculate framerate in video stream
    ...
    CDX_S32			width, height;      /**< pixel width and height, is not the picture width and height (maybe they are the same size)*/
...
typedef struct MOVFragment
{
    ...
    uint64_t base_data_offset;  // the 'moof' box offset in the file, used for get the media data offset
