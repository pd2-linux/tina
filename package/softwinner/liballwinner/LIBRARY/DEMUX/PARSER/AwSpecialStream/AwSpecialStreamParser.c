...
static cdx_int32 AwSpecialStreamParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *pMediaInfo)
{
    ...
    if(1)
    {
        ...
        logd("special parser test: %x,%x,%x,%x,%x,%x,%x,%x", pp[0],pp[1],pp[2],pp[3],pp[4],pp[5],pp[6],pp[7]);
    ...
    logd(" special parser get special data len: %d, width: %d, height: %d, codex: %x , bIs3DStream: %d", nSpecialDataLen, width, height, (unsigned int)codec, bIs3DStream);
    if(ret < 0)
        return -1;
    ...
    pMediaInfo->program[0].video[0].pCodecSpecificData = specialStreamParser->tempVideoInfo.pCodecSpecificData;
    pMediaInfo->program[0].video[0].nCodecSpecificDataLen = specialStreamParser->tempVideoInfo.nCodecSpecificDataLen;
...
static cdx_int32 AwSpecialStreamParserPrefetch(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    int64_t nPts = 0;
    int nStreamIndex = 0;
    if(specialStreamParser->status != CDX_PSR_IDLE && specialStreamParser->status != CDX_PSR_PREFETCHED)
    {
        CDX_LOGW("status != CDX_PSR_IDLE && status != CDX_PSR_PREFETCHED, BdParserPrefetch invaild");
...
static cdx_int32 AwSpecialStreamParserRead(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    else
    {
        ...
        ret |= CdxStreamRead(specialStreamParser->file, pkt->ringBuf, pkt->length - pkt->buflen);
    }
    if(ret <= 0)
    {
         return -1;
...
cdx_int32 AwSpecialStreamParserForceStop(CdxParserT *parser)
{
    ...
    while(specialStreamParser->status != CDX_PSR_IDLE && specialStreamParser->status != CDX_PSR_PREFETCHED)
    {
        usleep(10000);
    }
    specialStreamParser->status = CDX_PSR_IDLE;
...
cdx_uint32 AwSpecialStreamParserAttribute(CdxParserT *parser) /*return falgs define as open's falgs*/
{
	SpecialStreamParser *specialStreamParser = (SpecialStreamParser *)parser;
