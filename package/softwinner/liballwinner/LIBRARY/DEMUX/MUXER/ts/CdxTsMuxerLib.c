...
static void tsWriteBufferStream(CdxStreamT *pCdxStreamT, char *buf, int size)
{
	CdxStreamWrite(pCdxStreamT, buf, size);
    return;
}
    ...
    while (len > 0)
    {
        ...
        *q++ = 0x10 | s->cc;//do not write adaption section
		...
		if(context->output_buffer_mode != OUTPUT_CALLBACK_BUFFER)
		{
		    ...
		    {
                tsWriteBufferStream(context->stream_writer,(char*)ts->ts_read_ptr, TS_PACKET_SIZE*128);
...
static void tsWritePat(TsMuxContext *s)
{
    ...
    tsWriteSection1(&ts->pat, PAT_TID, ts->tsid, 1, 0, 0,
                          data, q - data);
}
...
cdx_int32 TsWritePacket(TsMuxContext *s, CdxMuxerPacketT *pkt)
{
	...
	if(st->codec.codec_id == MUX_CODEC_ID_H264)
	{
        ...
        if (s->pes_bufsize < total_size)
        {
            ...
            logd("<F:%s, L:%d> realloc pes_buffer [%d]->[%d]", __FUNCTION__, __LINE__, s->pes_bufsize, total_size+32*1024);
