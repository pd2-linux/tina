...
void demoParseArgument(encode_param_t *encode_param, char *argument, char *value)
{
    ...
    switch(arg)
    {
		...
		case DST_SIZE:
            ...
            break;
        case MP4:
            strcpy(encode_param->mp4_file, "write://");
            memset((encode_param->mp4_file + 8), 0, (sizeof(encode_param->mp4_file) - 8));
        	sscanf(value, "%255s", (encode_param->mp4_file + 8));
        	logd(" get mp4 file: %s ", encode_param->mp4_file);
...
int main(int argc, char** argv)
{
	...
	strcpy((char*)encode_param.intput_file,		"/mnt/UDISK/data/720p.yuv");
	strcpy((char*)encode_param.output_file,		"/mnt/UDISK/720p.264");
	strcpy((char*)encode_param.mp4_file,		"write:///mnt/UDISK/720p_h264.mp4");
	...
	mp4_path = encode_param.mp4_file;
    ...
    mp4stream.uri = strdup(mp4_path);
    logd("+++ mp4stream.uri: %s", mp4stream.uri);
    result = CdxStreamOpen(&mp4stream, NULL, NULL, &mp4writer, NULL);
    if(result < 0)
    {
    	loge("CdxStreamOpen failed");
    	return -1;
    }
    mux = CdxMuxerCreate(CDX_MUXER_MOV, mp4writer);
...
out:
	printf("output file is saved: %s and %s\n",encode_param.output_file, (encode_param.mp4_file + 8));
	fclose(out_file);
	out_file = NULL;
	fclose(in_file);
	in_file = NULL;
